...
export default function PaymentModal({ open, onClose, account, onPaymentSuccess }) {
  ...
  const accountId = account.loanAccountId || account.id;
  const rawEmi = account.monthlyEmi || account.emiAmount || account.monthlyEmiAmount || account.amount || 0;
  const payAmount = Number(rawEmi);

  const handlePayment = async () => {
    if (!accountId) {
      toast("Invalid Loan Account ID", "error");
      return;
    }
    ...
    try {
      ...
      const paymentOrder = await createOrderApi({
        amount: payAmount,
        loanAccountId: Number(accountId),
      });
      ...
      openRazorpayCheckout({
        ...
        user,
        onFailure: (error) => {
          toast(error.message || "Payment cancelled or failed", "error");
        },
        onSuccess: async (result) => {
          try {
            const verifyPayload = {
              razorpayOrderId: result.razorpay_order_id || result.razorpayOrderId || paymentOrder.orderId || paymentOrder.id,
              razorpayPaymentId: result.razorpay_payment_id || result.razorpayPaymentId || `pay_mock_${Date.now()}`,
              razorpaySignature: result.razorpay_signature || result.razorpaySignature || "mock-signature",
              loanAccountId: Number(accountId),
              amount: payAmount,
